      ...
      <div className="w-2/3 flex justify-end gap-5 ">
        {navlinks.map((link) => (
          <NavLink
            key={link.name}
            to={link.link}
            className={({ isActive }) => `${isActive ? "bg-background text-text border-2 border-text  " : "text-background bg-text"} text-sm  sm:flex justify-center items-center bg-text  rounded-3xl md:px-6 px-8 py-3  font-custom border-2 hover:text-text  ${show ? 'flex' : 'hidden'}`}
